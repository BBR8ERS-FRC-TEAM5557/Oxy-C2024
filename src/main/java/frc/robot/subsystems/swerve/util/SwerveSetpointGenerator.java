...
/**
 * This is 254s SwerveSetpointGenerator but modified to use WPILibs Geometry
 * classes and an
...
public class SwerveSetpointGenerator {
	...
	public SwerveSetpoint generateSetpoint(
			...
			final KinematicLimits kinematicLimits,
			final SwerveSetpoint prevSetpoint,
			ChassisSpeeds desiredState,
			double dt) {

		// Limit rotational velocity
		desiredState.omegaRadiansPerSecond = Math.signum(desiredState.omegaRadiansPerSecond)
				* Math.min(kinematicLimits.maxAngularVelocity(), Math.abs(desiredState.omegaRadiansPerSecond));

		// Limit translational velocity
		double velocity_magnitude = Math.hypot(desiredState.vxMetersPerSecond,
				desiredState.vyMetersPerSecond);
		if (velocity_magnitude > kinematicLimits.maxLinearVelocity()) {
			desiredState.vxMetersPerSecond = (desiredState.vxMetersPerSecond / velocity_magnitude)
					* kinematicLimits.maxLinearVelocity();
			desiredState.vyMetersPerSecond = (desiredState.vyMetersPerSecond / velocity_magnitude)
					* kinematicLimits.maxLinearVelocity();
		}
		...
		double domega = desiredState.omegaRadiansPerSecond - prevSetpoint.chassisSpeeds().omegaRadiansPerSecond;

		double max_velocity_step = kinematicLimits.maxLinearAcceleration() * Robot.defaultPeriodSecs;
		double min_translational_scalar = 1.0;

		double max_omega_step = kinematicLimits.maxAngularAcceleration() * Robot.defaultPeriodSecs;
		double min_omega_scalar = 1.0;

		if (max_velocity_step < Double.MAX_VALUE * Robot.defaultPeriodSecs) {
			// Check X
		...
		if (max_omega_step < Double.MAX_VALUE * Robot.defaultPeriodSecs) {
		...
		return generateSetpoint(limits, prevSetpoint, desiredState, dt);
	}
	...
	public SwerveSetpoint generateSetpoint(
		...
		for (int i = 0; i < modules.length; ++i) {
			...
			if (epsilonEquals(prevSetpoint.moduleStates()[i].speedMetersPerSecond, 0.0)) {
				...
				var necessaryRotation = GeometryUtil.inverse(prevSetpoint.moduleStates()[i].angle)
		...
		for (int i = 0; i < modules.length; ++i) {
			...
			final var deltaRotation = GeometryUtil.inverse(prevSetpoint.moduleStates()[i].angle)
